...
class AutoSyncMonitor {
  ...
  private async checkAndFixSyncIssues(): Promise<void> {
    ...
    for (const client of clients) {
      try {
        // 1. VERIFICAR CIERRE AUTOMÁTICO DIARIO A LAS 23:59
        await this.checkDailyAutomaticClosure(client.id);
  ...
  private async detectPaidOrdersInDebt(clientId: number): Promise<PaidOrderInDebt[]> {
    ...
    try {
      ...
      for (const debt of activeDebts) {
        ...
        // Verificar si la orden está marcada como "pagado completo"
  ...
  /**
   * Verifica si es necesario hacer cierre automático diario
   * Se ejecuta cada 5 segundos y detecta TODOS los días sin cierre desde la fecha de creación
   */
...
export function startAutoSyncMonitor(): void {
  autoSyncMonitor.start();
}

// Función para detener el monitor
export function stopAutoSyncMonitor(): void {
  autoSyncMonitor.stop();
}

// Función para obtener el estado
export function getAutoSyncStatus(): any {
